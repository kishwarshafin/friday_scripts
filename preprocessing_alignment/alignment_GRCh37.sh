...
ALIGN_FASTQS() {
  ...
  REF=/data/ref/GRCh37/human_g1k_v37.fasta
  ...
  do
    ...
    do
      ...
      do
        ...
        samtools sort -@32 -o ${READ_GROUP_NAME}_GRCh37_clean.bam -) > "/data/tmp/logs/bwa_samtools_HG001_GRCh37.log" 2>&1
        wait;

        mv ${READ_GROUP_NAME}_GRCh37_clean.bam ../
        echo "DONE: ${READ_GROUP}"
        cd ..
      done
      for i in *_GRCh37_clean.bam;do echo "INPUT=${i}" >> all_read_groups.txt ;done ;wait
      ...
      $(echo ${bam_array[@]}) \
      OUTPUT=${LIBRARY_NAME}_HG001_GRCh37.bam \
      METRICS_FILE=/data/tmp/logs/dupmetrics_${LIBRARY_NAME}_HG001_GRCh37.txt \
      CREATE_INDEX=true \
      ...
      TMP_DIR=/data/tmp) > "/data/tmp/logs/markdup_HG001_GRCh37.log" 2>&1
      wait;

      rm -rf *_GRCh37_clean.bam
      cd ..
...
# gsutil -o GSUtil:parallel_composite_upload_threshold=150M cp <bam_file> gs://kishwar-giab-alignments/bams/giab_HG001_GRCh37/
